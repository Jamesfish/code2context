/**
 * `contextforge init` command — Scan project and generate structured context.
 */
...
export async function initCommand(options: InitOptions): Promise<void> {
    ...
    if (options.ai) {
        ...
        if (aiAnalyzer.isAvailable()) {
            ...
            console.log(chalk.yellow('  ⚠ AI analysis skipped (set CONTEXTFORGE_API_KEY to enable)'));
            console.log(chalk.dim('    Supports: OpenAI, DeepSeek, OpenRouter, Ollama, or any OpenAI-compatible API'));
            console.log(chalk.dim('    See: https://github.com/Jamesfish/contextforge#ai-enhanced-analysis'));
        }
    }
    ...
    console.log(`  📁 ${chalk.cyan('.contextforge/context.json')} — Structured context (${(contextJson.length / 1024).toFixed(1)}KB)`);
    console.log(`  📄 ${chalk.cyan('.contextforge/context.md')} — Human-readable context`);
    console.log(`  ⏱️  Completed in ${(duration / 1000).toFixed(1)}s`);
    console.log('');
    console.log(chalk.dim('  Next steps:'));
    console.log(chalk.dim('    contextforge export --format cursor   → Generate .cursorrules'));
    console.log(chalk.dim('    contextforge export --format claude   → Generate CLAUDE.md'));
    console.log(chalk.dim('    contextforge update                   → Update after code changes'));
    console.log('');
}
...
function generateContextMarkdown(ctx: ProjectContext): string {
    ...
    lines.push('');
    lines.push(`> Generated by ContextForge on ${ctx.generated.timestamp}`);
    lines.push('');
